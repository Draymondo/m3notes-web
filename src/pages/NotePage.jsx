...
import { ArrowLeft, Pin, Archive, Trash2, ListChecks, AlignLeft, Tag, Copy, Share2, Star, History, X } from 'lucide-react'
  ...
  if (note.loadError) {
    return (
      <div className="note-page loading">
        <p>{note.loadError}</p>
        <button className="icon-btn" onClick={() => window.history.back()}>Retour</button>
      </div>
    )
  }
    ...
    <div className={`note-page ${colorClass[note.color] || 'note-default'}`}>
      <header className="note-toolbar">
        ...
        <div className="toolbar-actions">
          <button className="icon-btn" onClick={() => note.isChecklist ? note.switchToText() : note.switchToChecklist()} title={note.isChecklist ? 'Passer en texte' : 'Passer en checklist'}>
      ...
      <input className="note-title-input" placeholder="Titre" value={note.title} onChange={e => note.setTitle(e.target.value)} autoFocus={note.isNew} />
      <LabelEditor labels={note.labels} open={labelInputOpen} onAdd={note.addLabel} onRemove={note.removeLabel} />
      ...
      {note.historyOpen && (
        <div className="history-overlay" role="dialog" aria-modal="true" aria-label="Historique de la note">
          <div className="history-panel">
            <div className="history-header">
              <div><h2>Historique</h2><p>Versions précédentes de cette note</p></div>
              <button className="icon-btn" onClick={() => note.setHistoryOpen(false)} title="Fermer"><X size={20} /></button>
            </div>
            {note.historyError && <p className="note-error">{note.historyError}</p>}
            {note.historyLoading ? <p className="history-empty">Chargement…</p> : note.history.length === 0 ? (
              <p className="history-empty">Aucune ancienne version pour le moment.<br />Une version sera créée à chaque modification enregistrée.</p>
            ) : (
              <div className="history-list">
                {note.history.map(version => (
                  <article className="history-item" key={version.id}>
                    <div className="history-item-info">
                      <strong>{version.title || 'Sans titre'}</strong>
                      <span>{formatHistoryDate(version.savedAt)}</span>
                      <p>{version.isChecklist ? `${version.checklist?.length || 0} élément(s) de checklist` : (version.content || 'Note sans contenu')}</p>
                    </div>
                    <button className="history-restore-btn" onClick={() => note.restoreVersion(version)}>Restaurer</button>
                  </article>
                ))}
              </div>
            )}
